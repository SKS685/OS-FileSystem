...
struct ext_extent
{
    uint32_t ee_logical_block; // E.g., This extent covers file block offset 0
    uint32_t ee_start_block;   // E.g., The physical block on disk is 5000
    uint16_t ee_length;        // E.g., The next 1024 blocks belong to this file
...
struct ext_inode
{
    ...
    uint32_t i_atime;       // Access time (POSIX epoch)
    uint32_t i_ctime;       // Creation time
    uint32_t i_mtime;       // Modification time
    uint16_t i_gid;         // Group ID
    uint16_t i_links_count; // Hard links (when 0, file is deleted)
    uint32_t i_blocks;      // Total 4 KiB blocks actually allocated

    // Extent Payload
    // If the file is highly fragmented, you would use an Extent Tree (B-Tree).
    // For this project scope, a flat array of 4 extents inside the Inode
