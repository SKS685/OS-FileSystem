...
struct ext_super_block
{
    ...
    uint32_t s_total_blocks; // ~1.07 billion for 4 TiB
    uint32_t s_total_inodes;
    uint32_t s_free_blocks_count;
    uint32_t s_free_inodes_count;
    uint32_t s_blocks_per_group; // Always 32768
    uint32_t s_inodes_per_group;
    uint32_t s_bg_desc_start_block; // Where the Block Group Descriptor table begins

    uint8_t s_padding[4064]; // Pad out to exactly 4096 bytes (1 Block)
};
